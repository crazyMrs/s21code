...
START_TEST(create_normal) {
  ...
  if (matrix.matrix) {
    ...
    ck_assert_int_eq(status1, OKAY);
  } else {
    ck_assert_int_eq(status1, ERROR);
  }

  if (status1 == OKAY) s21_remove_matrix(&matrix);
}
END_TEST
...
START_TEST(create_no_rows) {
  ...
  const int cols = 10;
  matrix_t m = {0};
  ck_assert_int_eq(s21_create_matrix(rows, cols, &m), NOCORRECT);
}
END_TEST
...
START_TEST(create_NOCORRECT) {
  int rows = -10;
  int columns = 10;
  matrix_t matrix = {0};
  int status = s21_create_matrix(rows, columns, &matrix);

  ck_assert_int_eq(status, NOCORRECT);

  if (status == OKAY) s21_remove_matrix(&matrix);
}
END_TEST
...
START_TEST(sum_matrix) {
  ...
  ck_assert_int_eq(s21_sum_matrix(&m, &mtx, &res), OKAY);
...
START_TEST(sum_test_NOCORRECT_matrix) {
  matrix_t A, B, R;
  s21_create_matrix(1, 1, &A);
  s21_create_matrix(2, 1, &B);
...
START_TEST(sub_test_NOCORRECT_matrix) {
  matrix_t A, B, R;
  s21_create_matrix(1, 1, &A);
  s21_create_matrix(2, 1, &B);
...
START_TEST(transpose_test_5) {
  ...
  s21_remove_matrix(&A);
  s21_remove_matrix(&result);
  ck_assert_int_eq(NOCORRECT, result_transpose);
}
END_TEST
...
START_TEST(null_compliments) {
  ...
  ck_assert_int_eq(res, NOCORRECT);
}
END_TEST
...
START_TEST(empty) {
  matrix_t a = {0};
  matrix_t b = {0};
  ck_assert_int_eq(s21_calc_complements(&a, &b), NOCORRECT);
}
END_TEST
...
START_TEST(inverse_NOCORRECT) {
  matrix_t m = {0};
  matrix_t result = {0};
  int code = s21_inverse_matrix(&m, &result);
  ck_assert_int_eq(code, NOCORRECT);
}
END_TEST
...
*s21_matrix_test(void) {
  ...
  tcase_add_test(tCase, create_NOCORRECT);
  ...
  tcase_add_test(tCase, sum_test_NOCORRECT_matrix);
  ...
  tcase_add_test(tCase, sub_test_NOCORRECT_matrix);
  ...
  tcase_add_test(tCase, inverse_NOCORRECT);
